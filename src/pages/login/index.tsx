    ...
    return (
        <>
            ...
            <div className="flex w-full m-auto mt-0 shadow-xl lg:w-4/12 md:w-10/12 md:mt-28 bg-base-200 rounded-xl" >
                    ...
                    <p className="m-10 mx-auto text-lg font-bold text-center ">TravelZap</p>
                    <div className="box-border flex flex-wrap">
                        <button onClick={() => changeForm('login')} className={`flex-1 w-64 p-4 mr-4 text-center rounded-lg uppercase font-bold ${FormType === 'login' ? 'bg-primary text-white' : ''}`}>Sign in</button>
                        <button onClick={() => changeForm('register')} className={`flex-1 w-64 p-4 text-center rounded-lg uppercase font-bold ${FormType === 'register' ? 'bg-primary text-white' : ''}`}>Sign up</button>
                    </div>
                    {FormType && FormType === 'login' ?
                        ...
                        >
                            {({ isSubmitting, errors, touched }: FormikState<LoginValues>) => (
                                <Form>
                                    ...
                                    <button type="submit" disabled={isSubmitting} className="w-full btn bg-primary text-white">
                                        Submit
                                    </button>
                                </Form>
                            )}
                        ...
                        >
                            {({ isSubmitting, errors, touched }: FormikState<RegisterValues>) => (
                                <Form>
                                    ...
                                    <button type="submit" disabled={isSubmitting} className={`w-full btn bg-primary text-white ${isSubmitting ? 'btn loading' : ''}`}>
                                        Submit
                                    </button>
                                </Form>
                            )}
                        ...
                        <section className="box-border flex flex-wrap">
                            <SocialLogin className="w-64 mr-4" provider="Google" url={`${SERVER_URL}/google`}/>
                            <SocialLogin className="w-64" provider="Facebook" url={`${SERVER_URL}/facebook`}/>
                        </section>
                    </div>
            </div>    
            <div className="flex w-full m-auto mt-8 lg:w-4/12 md:w-10/12">
                <Link href="/" className="m-auto mb-10 text-xl shadow-xl btn btn-sm rounded-btn lg:m-0 btn-primary text-white">
                    <>
                        <FiArrowLeft/> Home
                    </>
                </Link>
